...
export function getAllReminders(req: Request, res: Response) {
  try {
    ...
    res.status(500).json({ success: false, error: 'Hatırlatıcılar yüklenemedi' });
  }
}
...
export function getReminderById(req: Request, res: Response) {
  try {
    ...
    if (!reminder) {
      return res.status(404).json({ success: false, error: 'Hatırlatıcı bulunamadı veya bu okula ait değil' });
    }
    
    res.json(reminder);
  } catch (error) {
    console.error('Error fetching reminder:', error);
    res.status(500).json({ success: false, error: 'Hatırlatıcı yüklenemedi' });
  }
}
...
export function getPendingReminders(req: Request, res: Response) {
  try {
    ...
    res.status(500).json({ success: false, error: 'Bekleyen hatırlatıcılar yüklenemedi' });
  }
}
...
export function createReminder(req: Request, res: Response) {
  try {
    ...
    res.status(500).json({ success: false, error: 'Hatırlatıcı oluşturulamadı' });
  }
}
...
export function updateReminder(req: Request, res: Response) {
  try {
    ...
    if (result.notFound) {
      return res.status(404).json({ success: false, error: 'Hatırlatıcı bulunamadı' });
    }
    
    res.json({ success: true });
  } catch (error) {
    console.error('Error updating reminder:', error);
    res.status(500).json({ success: false, error: 'Hatırlatıcı güncellenemedi' });
  }
}
...
export function updateReminderStatus(req: Request, res: Response) {
  try {
    ...
    res.status(500).json({ success: false, error: 'Hatırlatıcı durumu güncellenemedi' });
  }
}
...
export function deleteReminder(req: Request, res: Response) {
  try {
    ...
    res.status(500).json({ success: false, error: 'Hatırlatıcı silinemedi' });
  }
}
